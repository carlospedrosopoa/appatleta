...
Atualiza uma partida (principalmente usado para atualizar placar).
...
```json
{
  ...
  "tiebreakTime1": 7,
  "tiebreakTime2": 5,
  "cardUrl": null  // Invalida card antigo (será regenerado na próxima visualização)
}
```

**Nota:** Quando o placar é atualizado, o campo `cardUrl` é definido como `null` para invalidar o card antigo. O card será regenerado automaticamente na próxima visualização.
...
Gera ou retorna o card/imagem da partida para compartilhamento.
...
**Fluxo:**
1. Verifica se `cardUrl` existe no banco de dados
2. Se existe e arquivo está no GCS: retorna card do cache (rápido)
3. Se não existe ou foi invalidado: gera novo card, salva no GCS, atualiza `cardUrl` no DB, retorna card

**Response 200:**
- **Content-Type:** `image/png`
- **Body:** Blob/ArrayBuffer da imagem PNG

**Response 404:**
- Partida não encontrada

**Nota:** O card é gerado no backend usando imagens dos atletas do Google Cloud Storage. A URL do card gerado é armazenada no campo `cardUrl` da tabela `Partida` para cache e performance.

**Erros:**
- `401`: Não autenticado
- `404`: Partida não encontrada
- `500`: Erro ao gerar card ou acessar Google Cloud Storage
